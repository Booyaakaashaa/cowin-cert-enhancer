...
# Read Cert and convert to PNG
f_name = "cert.pdf"
pdf_file = fitz.open(f_name)
page = pdf_file[0]
pix = page.get_pixmap(alpha=False)
pix.save("cert.png")

img = cv.imread("cert.png")

img[600:698, 35:342] = [239, 236, 232]  # Remove PM Photo

img[697:699, 36:342] = [221, 201, 184]  # Repair Line
...
# Sketchify the user image
# img3 = cv.imread('user.jpg', 0)
img3 = cv.resize(cv.imread('user.jpg', 0), (136, 136), interpolation=cv.INTER_AREA)  # read user image and resize
img_inv = 255 - img3
img_blur = cv.GaussianBlur(img_inv, (21, 21), 0)
output3 = cv.divide(img3, 255 - img_blur, scale=256)
output3 = cv.cvtColor(output3, cv.COLOR_GRAY2RGB)

output[562: 698, 122: 258] = (output[562: 698, 122: 258] / 255) * (output3[:, :] / 255) * 255  # OVerlap user photo

# img4 = cv.imread('stamp.png')
# Overlap stamp art on the image
img4 = cv.resize(cv.imread('stamp.png'), (315, 300), interpolation=cv.INTER_AREA)
output[475: 775, 0: 315] = (output[475: 775, 0: 315] / 255) * (img4[:, :] / 255) * 255
cv.imwrite('output.png', output)

# Create PDF from image
img_pdf = fitz.open()
image = fitz.open('output.png')
shape = image[0].rect
pdf_bytes = image.convert_to_pdf()
image.close()
